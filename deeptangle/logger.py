...
def start_logging(flags, model_dir: str):
    ...
    training_flags = [
        v for k, v in flags.flags_by_module_dict().items() if "train" in k
    ]
    ...
    if flags.save:
        ...
        logging.info(
            "Experiment logs stored at %s/%s.INFO", experiment_dir, info_flags["uid"]
        )
    else:
        experiment_dir = None
...
def time_activity(activity_name: str):
    ...
    print("[Timing] %s finished: %.4f s.", activity_name, duration)
