...
class SplineSheet:
    def __init__(self, M):
        ...
        if (
            M[0] >= self.splineGenerator[0].support
            and M[1] >= self.splineGenerator[1].support
        ):
            self.M = M
        else:
            raise RuntimeError(
                "Each M must be greater or equal than its spline generator support size."
            )

        self.halfSupport = (
            self.splineGenerator[0].support / 2.0,
            self.splineGenerator[1].support / 2.0,
        )
        self.controlPoints = numpy.zeros(
            (
                (self.M[1] + int(self.splineGenerator[1].support))
                * (self.M[0] + int(self.splineGenerator[0].support)),
                3,
            )
        )
    ...
    def initializeFromPoints(self, samplePoints, sampleParameters):
        ...
        if len(samplePoints) != len(sampleParameters):
            raise RuntimeError(
                "samplePoints and sampleParameters must be the same length."
            )
    ...
    def sample(self, samplingRate):
        ...
        paramlist = [
            (float(x[0]) / samplingRate[0], float(x[1]) / samplingRate[1])
            for x in samplinglist
        ]
                            ...
                            point = point + (
                                self.controlPoints[
                                    ...
                                    + (
                                        ...
                                        * (
                                            self.M[1]
                                            + int(self.splineGenerator[1].support)
                                        )
                                    )
                                ]
                                * self.splineGenerator[0].firstDerivativeValue(sVal)
                                * self.splineGenerator[1].firstDerivativeValue(tVal)
                            )
                        elif ds:
                            point = point + (
                                self.controlPoints[
                                ...
                                * self.splineGenerator[0].firstDerivativeValue(sVal)
                                * self.splineGenerator[1].value(tVal)
                            )
                        elif dt:
                            point = point + (
                                self.controlPoints[
                                ...
                                * self.splineGenerator[0].value(sVal)
                                * self.splineGenerator[1].firstDerivativeValue(tVal)
                            )
                        else:
                            point = point + (
