...
def isWormHTSwitched(
    skeletons, segment4angle, max_gap_allowed, window_std, min_block_size
):
    """
...
def correctHeadTail(skeletons_file, **params):
    ...
    base_name = (
        skeletons_file.rpartition(".")[0].rpartition(os.sep)[-1].rpartition("_")[0]
    )
    ...
    with pd.HDFStore(skeletons_file, "r") as ske_file_id:
        ...
        rows_indexes = indexes_data.groupby("worm_index_joined").agg([min, max])[
            "skeleton_id"
        ]
        del indexes_data
    ...
    for ii, dat in enumerate(rows_indexes.iterrows()):
        ...
        worm_data = WormClass(
            skeletons_file, worm_index, rows_range=(row_range["min"], row_range["max"])
        )
